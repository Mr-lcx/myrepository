...
class Panel_2_2(MyPanel):
    ...
    def OnSubmit(self, event):
        ...
        if self.input2.GetValue() != self.input3.GetValue():
            ...
            return
        if self.person.type == "admin" and self.db.loginAdmin(self.person.id, self.input1.GetValue()):
            i = self.db.adminUpdatePassword(self.person.id, self.input2.GetValue())
            self.db.commit()
            if i > 0:
                ...
                self.input3.Clear()
            return
        if self.person.type == "student" and self.db.loginStudent(self.person.id, self.input1.GetValue()):
            i = self.db.adminUpdatePassword(self.person.id, self.input2.GetValue())
            self.db.commit()
            if i > 0:
                self.status.SetLabel("修改成功")
                ...
                self.input3.Clear()
            return
        if self.person.type == "teacher" and self.db.loginTeacher(self.person.id, self.input1.GetValue()):
            i = self.db.adminUpdatePassword(self.person.id, self.input2.GetValue())
            self.db.commit()
            if i > 0:
                self.status.SetLabel("修改成功")

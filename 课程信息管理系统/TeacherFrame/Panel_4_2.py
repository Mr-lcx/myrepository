...
class Panel_4_2(MyPanel):
    def __init__(self, parent):
        ...
        self.buttonOutPut = wx.Button(self, label="导出成绩文件", pos=(680, 200), size=(100, 80))
        self.buttonImport.Bind(wx.EVT_BUTTON, self.Import)
        self.buttonOutPut.Bind(wx.EVT_BUTTON, self.OutPut)
    ...
    def Search(self,event):
        ...
        print("筛选++++",selectNum)
        self.writeFileName=str(selectNum)+"课程成绩单"
    ...
    def Import(self,event):
        if self.flag:# 正式导入成绩！！！
            ...
            for item in self.data:
                ...
                # fg = self.db.InsertStudents(tuple(item))
                print("****************",fg)
                if fg:
    ...
    def OutPut(self,event):
        try:  # 导出成绩单
            readfile = ReadExcle()
            readfile.write(self.data,self.writeFileName)
            wx.MessageBox("成绩单导出成功，请查看")
        except Exception as e:
            wx.MessageBox("成绩单导出错")

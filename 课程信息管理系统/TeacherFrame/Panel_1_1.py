...
class Panel_1_1(MyPanel):
    def __init__(self, parent):
        ...
        s = "欢迎您：   教工号: %s , 姓名: %s ,性别：%s 身份: %s" % (self.person.id,self.person.name, self.person.sex, self.person.type)
        ...
        self.input4 = wx.TextCtrl(self, pos=(120, 170), size=(200, 32),value=self.person.phone)
    ...
    def OnSubmit(self, event):
        phone = self.input4.GetValue()
        xueyuan=self.input5.GetValue()
        fg = self.db.teacherUpdate(self.id, phone,xueyuan)
        if fg: # 修改成功
